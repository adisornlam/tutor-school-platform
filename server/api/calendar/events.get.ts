...
export default defineEventHandler(async (event) => {
  ...
  // Get user's branch_ids (for branch admin, tutor, student, parent)
  ...
  if (isBranchAdmin && !isSystemAdmin && !isAdmin) {
    ...
    // Get parent's children branch_ids through enrollments
  ...
  // Build conditions for shared events access
  ...
  let sql = `
    ...
    WHERE (
      ...
      ${includeShared && sharedScopeConditions.length > 0 
        ? `OR (ce.is_shared = TRUE AND (${sharedScopeConditions.join(' OR ')}))` 
        : ''}
    )
  `
