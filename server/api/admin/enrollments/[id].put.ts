...
export default defineEventHandler(async (event) => {
  ...
  // Validate enrollment_type if provided
  ...
  const enrollmentType = body.enrollment_type !== undefined ? body.enrollment_type : oldEnrollment.enrollment_type || 'onsite'
  ...
  try {
    ...
    // Validate branch if provided (only for onsite enrollment)
    ...
    // Validate shipping_address_id if provided (only for online enrollment)
    ...
    if (body.branch_id !== undefined) {
      // If changing to online, set branch_id to null
      // If changing to onsite, set branch_id
    ...
    if (body.shipping_address_id !== undefined) {
      // If changing to onsite, set shipping_address_id to null
      // If changing to online, set shipping_address_id
    ...
    // Update course_branches current_enrollments if status changed (only for onsite enrollments)
    if (body.status && body.status !== oldEnrollment.status) {
      const finalEnrollmentType = body.enrollment_type !== undefined ? body.enrollment_type : (oldEnrollment.enrollment_type || 'onsite')
