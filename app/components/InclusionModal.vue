    ...
    <div class="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold">
          {{ inclusion ? 'แก้ไขสิ่งที่ได้รับ' : 'เพิ่มสิ่งที่ได้รับ' }}
        </h2>
        <button
          @click="$emit('close')"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.inclusion) {
      await $fetch(`${config.public.apiBase}/admin/settings/inclusions/${props.inclusion.id}`, {
        method: 'PUT',
        headers: {
      ...
      await $fetch(`${config.public.apiBase}/admin/settings/inclusions`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken.value}`
        },
