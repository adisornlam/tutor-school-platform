<template>
  <div class="bg-white rounded-lg shadow p-6">
    <h3 class="text-lg font-semibold mb-4">การตั้งค่า Ksher Thailand</h3>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            type="url"
            placeholder="https://api.ksher.com"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
          >
        </div>
      </div>
      ...
      <div class="flex items-center space-x-4">
        <label class="flex items-center">
          ...
          >
          <span class="ml-2 text-sm text-gray-700">Test Mode (ใช้ Sandbox)</span>
        </label>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.error('Error saving Ksher config:', err)
