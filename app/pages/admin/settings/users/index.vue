      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          >
            ...
            <option value="branch_admin">Admin สาขา</option>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <div class="flex items-center justify-end space-x-2">
                ...
                <button
                  ...
                  @click.stop="confirmUpdateStatus(user.id, 'inactive', 'ปิดใช้งาน')"
                ...
                <button
                  v-else
                  @click.stop="confirmUpdateStatus(user.id, 'active', 'เปิดใช้งาน')"
...
const loadUsers = async () => {
  ...
  try {
    ...
    if (response.success) {
      // Filter out users that only have parent or student roles
      users.value = response.data.filter(user => {
        const hasValidRoles = user.roles.some(role => 
          ['system_admin', 'owner', 'admin', 'branch_admin', 'tutor'].includes(role)
        )
...
const confirmUpdateStatus = async (userId: number, newStatus: string, actionName: string) => {
  ...
  const confirmed = await confirm({
    ...
    message: `คุณแน่ใจหรือไม่ว่าต้องการ${actionName}ผู้ใช้งาน ${user.first_name} ${user.last_name}?`,
...
const confirmDelete = async (user: User) => {
  ...
  const confirmed = await confirm({
    title: 'ยืนยันการลบ',
    message: `คุณแน่ใจหรือไม่ว่าต้องการลบผู้ใช้งาน ${user.first_name} ${user.last_name}?\nการกระทำนี้ไม่สามารถยกเลิกได้`,
    confirmText: 'ลบ',
    cancelText: 'ยกเลิก',
    type: 'danger'
  })
