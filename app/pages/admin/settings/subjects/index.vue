<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-3xl font-bold">จัดการวิชา</h1>
      <button
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
      <div>
        ...
        <input
          ...
          type="text"
          placeholder="ค้นหาด้วยชื่อวิชา, รหัส, ชื่อย่อ"
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div v-else-if="subjects.length === 0" class="p-8 text-center text-gray-500">
        ไม่พบวิชา
      </div>
      ...
      <table v-else class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="subject in subjects" :key="subject.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4">
              <div class="text-sm text-gray-500 max-w-xs truncate">{{ subject.description || '-' }}</div>
            </td>
    ...
    <!-- Create/Edit Modal -->
    <SubjectModal
      v-if="showCreateModal || editingSubject"
      :show="showCreateModal || !!editingSubject"
...
const loadSubjects = async () => {
  ...
  try {
    ...
    const response = await $fetch<{
      success: boolean
      data: Subject[]
    }>(`${config.public.apiBase}/admin/settings/subjects?${params.toString()}`, {
      headers: {
...
const confirmDelete = async (subject: Subject) => {
  ...
  const confirmed = await confirm({
    title: 'ยืนยันการลบ',
    message: `คุณแน่ใจหรือไม่ว่าต้องการลบวิชา ${subject.name}?\nการกระทำนี้ไม่สามารถยกเลิกได้`,
    confirmText: 'ลบ',
    cancelText: 'ยกเลิก',
    type: 'danger'
  })
  ...
  try {
    await $fetch(`${config.public.apiBase}/admin/settings/subjects/${subject.id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${accessToken.value}`
      }
    ...
    alert(err.data?.message || 'เกิดข้อผิดพลาดในการลบวิชา')
  }
}
